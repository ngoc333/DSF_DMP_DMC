    ...
    public partial class FORM_DMC_SET_BALANCE: Form
    {
            ...
            try
            {
                ...
                if (dt_temp != null & dt_temp.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        lblTotalSet.Text = "Total Set : " + totSet.ToString("#,###,###") + " Prs ";
                        lblCore.Text = "Total Core : " + totCore.ToString("#,###,###") + " Prs";
                        lblCarrier.Text = "Total Carrier : " + totCarrier.ToString("#,###,###") + " Prs";
                        ...
                        for (int i = 0; i < dtSource.Rows.Count - 1; i++)
                        {
                            for (int j = 0; j < dtSource.Columns.Count; j++)
                            {
                                ...
                                if (dtSource.Rows[i][0].ToString().Equals("Total Set"))
                                {
                                    ...
                                    axfpSpread.BackColor = Color.LightGray;
                                    axfpSpread.Font = new Font("Calibri", 13, FontStyle.Bold);
                                    lblTotalSet.Text = dtSource.Rows[i][3].ToString();
                                }
        ...
        private bool bindingDataSource(DataTable dtSource, DataTable dt)
        {
            try
            {
                ...
                {
                    if( dt.Rows[i]["COMPONENT_NAME"].ToString().ToUpper().Contains("CORE"))
                    {
                        totCore += Convert.ToDouble(dt.Rows[i]["QTY"].ToString());
                    }
                    if( dt.Rows[i]["COMPONENT_NAME"].ToString().ToUpper().Contains("CARRIER"))
                    {
                        totCarrier += Convert.ToDouble(dt.Rows[i]["QTY"].ToString());
                    }
                    if (dt.Rows[i]["COMPONENT_NAME"].ToString().ToUpper().Contains("SET"))
                    {
                        totSet += Convert.ToDouble(dt.Rows[i]["QTY"].ToString());
                    }
        ...
        {
            flg.Hide();
            Smart_FTY.ClassLib.WinAPI.AnimateWindow(flg.Handle, 2000, Smart_FTY.ClassLib.WinAPI.getSlidType("2"));
            flg.Show();
        }
