...
namespace Smart_FTY
{
    public partial class FRM_SMT_DMP_TOPO_DAILY : Form
    {
        public FRM_SMT_DMP_TOPO_DAILY()
        {
            InitializeComponent();
        }

        public FRM_SMT_DMP_TOPO_DAILY(string strChart)
        {
            week = true;
            InitializeComponent();
        }
        ...
        public DataTable SEL_OS_TOPO_WEEKLY(string ARG_QTYPE, string ARG_OP)
        {
            ...
            try
            {
                string process_name = "MES.PKG_SMT_B2.SEL_TOPO_WEEKLY";
            ...
            switch (ButtonCD)
            {
                ...
                case "C":
                    if (week)
                    {
                        ...
                        Form fc2 = Application.OpenForms["FRM_SMT_DMP_TOPO_WEEKLY"];
                        if (fc2 != null)
                            fc2.Show();
                        else
                        {
                            FRM_SMT_DMP_TOPO_WEEKLY f1 = new FRM_SMT_DMP_TOPO_WEEKLY();
                            f1.Show();
                        }
                    }
                    else
                ...
                case "D":
                    this.Hide();
                    Form fc = Application.OpenForms["FRM_SMT_DMP_TOPO_DAILY"];
                    if (fc != null)
                        fc.Show();
                    else
                    {
                        FRM_SMT_DMP_TOPO_DAILY f = new FRM_SMT_DMP_TOPO_DAILY();
                        f.Show();
                    }
                    break;
                case "W":
        ...
        private void LoadChart5(DataTable dt, string dept, ChartControl Cchart)
        {
            if (dt.Select("DEPT LIKE '" + dept + "%' AND DEPT NOT LIKE '" + dept + "-%'", "DIV").Count() > 0)
            {
                bindingchart(dt.Select("DEPT LIKE '" + dept + "%' AND DEPT NOT LIKE '" + dept + "-%'", "DIV").CopyToDataTable(), Cchart);
            }
            else
            {
                bindingchart(null, Cchart);
            }
        }
        ...
        private void BindingData(string arg_op)
        {
            try
            {
                ...
                {
                    // Cold Shot
                    LoadChart5(dtsource1, "IP-B COLD SHOT-INJECTION", chart1);
                    // DMP                      
                    LoadChart5(dtsource1, "IP-B DMP", chart2);
                    // trimming 
                    LoadChart5(dtsource1, "IP-B DMP-TRIMMING", chart3); 
                    //UV 
                    LoadChart5(dtsource1, "IP-B UV", chart4); 
                    //Spray 
                    LoadChart5(dtsource1, "IP-B NORMAL-SPRAY", chart5); 
                    //Hotknife 
                    LoadChart5(dtsource1, "IP-B HOT KNIFE", chart6); 
                    //lazer 
                    LoadChart5(dtsource1, "IP-B LASER", chart7); 
                    //Lazer Siping
                    LoadChart5(dtsource1, "IP-B LASER SIPING", chart8); 
                    //MSP Spray
                    LoadChart5(dtsource1, "IP-B MSP-SPRAY", chart9); 
                    //Zoom X Cutting
                    LoadChart5(dtsource1, "IP-B ZOOM X CUTTING", chart10); 
                    //Support 
                    LoadChart5(dtsource1, "IP-B SUPPORT", chart11);                     
                    //TOTAL
