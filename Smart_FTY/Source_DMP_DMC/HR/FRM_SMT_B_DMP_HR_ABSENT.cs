...
namespace Smart_FTY
{
    public partial class FRM_SMT_B_DMP_HR_ABSENT : SampleFrm2
    {
        public FRM_SMT_B_DMP_HR_ABSENT()
        {
            InitializeComponent();
            tmrTime.Stop();
            tmrLoad.Stop();
        ...
        private void loadDataGridAbsent(DataTable argDt)
        {
            try
            {
                ...
                for (int i = 0; i < argDt.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < arr.Length; j++)
                    {
                        ...
                        if (j + 1 > 2)
                        {
                            ...
                            axfpAbsent.TypeHAlign = TypeHAlignConstants.TypeHAlignRight;
                            axfpAbsent.TypeVAlign = TypeVAlignConstants.TypeVAlignCenter;
                        }
                        else if (j + 1 == 1)
                        {
                            axfpAbsent.BackColor = Color.Gray;
                            axfpAbsent.ForeColor = Color.White;
                            axfpAbsent.TypeHAlign = TypeHAlignConstants.TypeHAlignCenter;
                            axfpAbsent.TypeVAlign = TypeVAlignConstants.TypeVAlignCenter;
                        }
                        else if (j + 1 == 2)
                        {
                ...
                axfpAbsent.SetCellBorder(iNumRow + 3, 1, iNumRow + 3, axfpAbsent.MaxRows, CellBorderIndexConstants.CellBorderIndexBottom, 0, CellBorderStyleConstants.CellBorderStyleBlank);
        ...
        private void loadData()
        {
            try
            {
                System.Data.DataSet ds = GET_DATA("DMP");
        ...
        private void FRM_SMT_HR_ABSENT_Load(object sender, EventArgs e)
        {
            ...
            lblTitle.Text = "DMP Human Attendance";
